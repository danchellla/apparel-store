...
const Sale = () => {
  ...
  let saleProducts = products.filter(p => p.category === 'sale');
  if (sizeFilter) {
    saleProducts = saleProducts.filter(p => p.sizes && p.sizes.includes(sizeFilter));
  }
  if (sort === 'price-asc') {
    saleProducts = [...saleProducts].sort((a, b) => a.price - b.price);
  } else if (sort === 'price-desc') {
    saleProducts = [...saleProducts].sort((a, b) => b.price - a.price);
  } else if (sort === 'name') {
    saleProducts = [...saleProducts].sort((a, b) => a.name.localeCompare(b.name));
  }
  ...
  return (
    ...
    >
        ...
        <div className="w-full max-w-2xl bg-glass-bg rounded-xl shadow-lg p-4 flex flex-col sm:flex-row gap-4 items-center justify-between border border-accent">
          ...
          <div className="flex flex-col flex-1 min-w-[180px]">
            ...
            >
              ...
              {[...new Set(products.filter(p => p.category === 'sale' && p.sizes).flatMap(p => p.sizes || []))].map(size => (
      ...
      {loading ? (
        ...
        <div className="text-center text-lg text-gray-400 mt-16">No products currently on sale.</div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {saleProducts.map(product => (
            <motion.div
              key={product.id}
              className="glass-card p-4 rounded-2xl shadow-xl hover:scale-105 transition-transform duration-300 border-2 border-accent"
